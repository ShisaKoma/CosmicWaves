...
(function(global){
 'use strict';
 const Branes=typeof module!=='undefined'&&module.exports?require('./local-branes.js'):global.LocalBranes;
 ...
 function defaults(n=10){return{version:5,positions:n,target:'#8B5CF6',seed:728931,
   ...
   life:{enabled:false,birth:[3,4],survival:Array.from({length:27},(_,i)=>i),inheritance:'point',protectCondensates:false},
   ...
   colors:{addition:'average',amplitudeLimit:64},collisions:{subtraction:'never',probability:.25},
   gravity:{enabled:true,trigger:'configuration',tolerance:.09,minimumPositions:1,requiredPositions:[],strength:3,radius:6,softening:1,spectralTolerance:1},
   emergence:{mode:'geometry'},branes:Branes.defaults(),radial:{enabled:true,rate:.2,unitRadius:.18},
 ...
 function validate(input){
   if(!input||typeof input!=='object'||Array.isArray(input)||!Number.isInteger(input.positions)||input.positions<2||input.positions>32)throw Error('Un objet de règles avec 2 à 32 positions entières est requis.');
   ...
   if(c.version===4){if(Object.hasOwn(c,'branes'))throw Error('Champ branes dans un fichier version 4.');c.version=5;c.branes=Branes.defaults();}
   ...
   keys(c,base,'règles');choice(c.version,[5],'version');if(!/^#[0-9a-f]{6}$/i.test(c.target))throw Error('target : couleur #RRGGBB requise.');c.target=c.target.toUpperCase();num(c.seed,1,4294967295,'seed',true);num(c.ticksPerSecond,.1,12,'ticksPerSecond');
   for(const key of ['grid','initial','weights','life','structures','colors','collisions','gravity','motion','condensation','emission','display','emergence','radial','breakup','branes'])keys(c[key],base[key],key);
   bool(c.branes.collective,'branes.collective');
   for(const k of ['asymmetry','emergenceTolerance','stabilityTolerance'])num(c.branes[k],0,1,'branes.'+k);
   num(c.branes.centralBranes,0,3,'branes.centralBranes',true);num(c.branes.captureFraction,0,.5,'branes.captureFraction');
   for(const k of ['stabilitySteps','holdSteps','minRegionVoxels'])num(c.branes[k],1,100,'branes.'+k,true);
   num(c.branes.minimumRadius,.01,2,'branes.minimumRadius');num(c.branes.shrinkRate,.001,1,'branes.shrinkRate');
   if(c.branes.stabilityTolerance>c.branes.emergenceTolerance)throw Error('Le seuil de stabilité doit être inférieur ou égal au seuil d’émergence.');
   if(c.emergence.mode==='geometry'&&c.grid.boundary!=='closed')throw Error('Le mode géométrique exige grid.boundary = closed.');
   ...
   choice(c.emergence.mode,['geometry','local-complete','loop-count','palette'],'emergence.mode');bool(c.radial.enabled,'radial.enabled');num(c.radial.rate,.001,1,'radial.rate');num(c.radial.unitRadius,.01,1,'radial.unitRadius');
 ...
 class World{
   ...
   describe(cell){
     ...
     const assembled=c.emergence.mode!=='geometry'&&shapeCounts[3]>=c.condensation.minimumLoops&&(c.emergence.mode==='local-complete'?neighborLoops===0:c.emergence.mode==='palette'?loopPositions.size>=c.condensation.requiredPositions:true);
     ...
     const source=c.emergence.mode!=='geometry'&&eligible&&c.gravity.enabled&&c.gravity.strength>0&&(!condensed||c.condensation.gravityMultiplier>0);
   ...
   refresh(){for(const p of this.cells.values())Object.assign(p,this.describe(p));if(this.pendingReclassification){this.syncEpisodes(this.pendingReclassification);this.pendingReclassification=null;}this.branes?.update(this);this.stats=this.summary();}
   summary(){let sources=0,eligible=0,configurations=0,spheres=0,units=0;const shapes=[0,0,0,0];for(const p of this.cells.values()){sources+=+p.source;eligible+=+p.eligible;configurations+=+p.configurationX;spheres+=+p.condensed;units+=p.units;for(let i=0;i<4;i++)shapes[i]+=p.shapeCounts[i];}const longestXAge=Math.max(0,...[...(this.activeEpisodes?.values()||[])].map(e=>this.generation-e.start));const captured=this.branes?.captured||0,preparationReserve=(this.preparationReserve||[]).reduce((a,b)=>a+b,0),domains=this.branes?.active()||[];units+=captured+preparationReserve;sources+=(this.branes?.sources(this)||[]).length;return{captured,preparationReserve,domains:domains.length,stableDomains:domains.filter(d=>d.status==='stable sur la fenêtre').length,domainEmergences:(this.branes?.domains||[]).filter(d=>d.born!==null&&!d.forced).length,forcedDomains:(this.branes?.domains||[]).filter(d=>d.born!==null&&d.forced).length,generation:this.generation,cells:this.cells.size,sources,eligible,configurations,spheres,units,longestXAge,totalEmergences:this.totalEmergences,points:shapes[0],halves:shapes[1],strings:shapes[2],loops:shapes[3],...this.events};}
   ...
   experiment(){return clone({format:'chromatic-experiment-v5',branes:this.branes.export(),preparationReserve:this.preparationReserve,config:this.config,ruleHistory:this.ruleHistory,interventions:this.interventions,episodes:this.episodes,history:this.history,current:this.stats,rngState:this.seed,cells:[...this.cells.values()],scope:{spatialDimensions:3,bosonDynamics:false,photonPropagation:false,thermalCosmology:false},note:'Épisodes de X par site, pas identités d’objets. Épisodes ouverts censurés à droite ; réglages et interventions censurent les durées. Bilan de constituants, pas énergie. Aucun taux physique. Export documentaire, non importable comme règles.'});}
   forceEmergence(){return this.branes.force(this);}
   openBreach(){
     const cell=[...this.cells.values()].find(p=>p.brane);
     if(!cell)throw Error('Préparez d’abord une fermeture pour ouvrir une brèche.');
     const counts=this.counts(cell.parts);this.preparationReserve=this.preparationReserve.map((v,i)=>v+counts[i]);this.cells.delete(cell.key);
     this.interventions.push({type:'open-breach',generation:this.generation,key:cell.key,displaced:counts,note:'Portion retirée vers la réserve de préparation, sans destruction.'});this.refresh();
   }
   reset(){this.branes=new Branes.Model();this.preparationReserve=Array(this.config.positions).fill(0);this.seed=this.config.seed;this.generation=0;this.totalEmergences=0;this.episodes=[];this.activeEpisodes=new Map();this.nextEpisodeId=1;this.interventions=[];this.pendingReclassification=null;this.history=[];this.ruleHistory=[{generation:0,config:clone(this.config)}];this.cells=new Map();this.clearEvents();const center=(this.n-1)/2;
   ...
   step(){
     ...
     const c=this.config;this.syncEpisodes('observed');this.clearEvents();const old=[...this.cells.values()].sort((a,b)=>a.key-b.key),unitsBefore=old.reduce((s,p)=>s+this.counts(p.parts).reduce((a,b)=>a+b,0),0)+this.branes.captured+this.preparationReserve.reduce((a,b)=>a+b,0),oldX=new Set(old.filter(p=>p.configurationX).map(p=>p.key)),sources=old.filter(p=>p.source).map(p=>({p,xyz:this.coordinates(p.key)})).concat(this.branes.sources(this)),neighbors=new Map();
     ...
     for(const a of sorted){if((c.emergence.mode==='geometry'&&a.brane)||held.has(a.key)||a.flight>0||this.random()>=c.structures.bondProbability)continue;for(const d of this.offsets){const k=this.neighbor(a.key,d),b=byKey.get(k);if(!b||(c.emergence.mode==='geometry'&&b.brane)||held.has(k)||b.flight>0||!this.canBond(a,b))continue;const ai=this.describe(a),bi=this.describe(b);if((ai.source||ai.condensed)&&(bi.source||bi.condensed))continue;const anchor=ai.source||ai.condensed?a:b,mover=anchor===a?b:a;bonds.set(mover.key,anchor.key);held.add(a.key);held.add(b.key);break;}}
     ...
     for(const {p,d,k}of proposals){if(!next.has(k)){next.set(k,this.cell(k,[],{...(p.brane?{brane:true}:{}),parts:p.parts,condensed:p.condensed,envelopeRadius:p.envelopeRadius,flight:p.flight,direction:[...d]}));continue;}const a=next.get(k);if(p.brane)a.brane=true;const opposed=a.direction.reduce((s,v,i)=>s+v*d[i],0)<0;const subtract=(c.collisions.subtraction==='any'||(c.collisions.subtraction==='opposed'&&opposed))&&this.random()<c.collisions.probability;a.parts=this.mergeParts(a.parts,p.parts,subtract);a.condensed=a.condensed||p.condensed;a.envelopeRadius=Math.cbrt(a.envelopeRadius**3+p.envelopeRadius**3);a.flight=0;a.direction=[0,0,0];this.events[subtract?'subtractions':'additions']++;if(!a.parts.length){next.delete(k);this.events.annihilations++;}}
     ...
     this.cells=next;this.generation++;this.branes.update(this,true);this.refresh();
     ...
     const unitsAfter=this.summary().units;
