...
(function(global){
 ...
 function validate(c){
   if(c&&c.version===1)c={...defaults(),...c,version:2,domainEnabled:false};
   ...
   if(!['random','prepared'].includes(c.initialLayout))throw Error('Disposition inconnue.');
 ...
 class World{
   constructor(c=defaults()){
     c=validate(c);this.config=c;this.seed=c.seed;this.generation=0;this.time=0;this.length=3.2;this.surfaces=[];this.sources=[];this.history=[];this.domains=[];this.events=[];this.nextDomainId=1;
   ...
   detect(advance=true){
     ...
     const enclosed=c.criterion==='enclosure'?enclosedMask(occupied,N,allColors):{mask:Uint8Array.from(occupied,x=>x===allColors?1:0),regions:0};
     ...
     for(let k=0;k<this.mask.length;k++)if(this.mask[k]&&!seen[k]){
       ...
       sources.push({key:k,center:sum.map(x=>x/queue.length),voxels:queue.length,cells:queue,charge:queue.length*dx**3});
     }
   ...
   createDomain(source){
     ...
     const d={id:this.nextDomainId++,bornStep:this.generation,internalTime:0,phase:'linked',center,patches,anchors,cells:[...source.cells],radius:c.minimumRadius,initialCharge:source.charge,targetPerSurface,captured:Array(c.colors).fill(0),breakStep:null,breakReason:null};
   ...
   syncDomains(){
     ...
     for(const source of this.sources){
       const cells=new Set(source.cells);let match=null,best=0;
       ...
       if(match){used.add(match.id);match.cells=[...source.cells];}else this.createDomain(source);
     }
   ...
   breakDomain(id,reason='manual',record=true){
     ...
     d.phase='broken';d.breakStep=this.generation;d.breakReason=reason;
   ...
   step(){
     ...
     this.detect();if(c.domainEnabled){this.syncDomains();this.domainEvolution();}this.stats=this.summary();this.history.push({...this.stats});return this.stats;
   }
   experiment(){return{format:'wave-surfaces-v2',domains:JSON.parse(JSON.stringify(this.domains)),events:JSON.parse(JSON.stringify(this.events)),contentBalance:this.contentBalance(),config:{...this.config},history:this.history.map(s=>({...s})),sources:this.sources.map(s=>({...s,center:[...s.center]})),surfaces:this.surfaces.map(s=>({...s,normal:[...s.normal],u:[...s.u],v:[...s.v],content:Array.from(s.content),h:Array.from(s.h),velocity:Array.from(s.velocity)})),ages:Array.from(this.ages),note:'Surfaces 2D à déplacement normal dans un espace 3D fixé. Fermeture discrète : région libre sans chemin extérieur à 26 voisins, bordée par toutes les couleurs. Alternative : croisement sans fermeture. Persistance mesurée par site, non par objet suivi. Charge attractive = volume qualifié, sans conversion d’énergie ni masse physique identifiée. Domaines v2 : sphère illustrative centrale, capture locale à parts égales, horloge conventionnelle démarrée à la naissance, rétrécissement après rupture des liaisons. Le paramètre numérique préalable ne démontre ni temps physique préalable ni absence de causalité. Aucun calcul de métrique ou de cosmologie.'};}
 }
